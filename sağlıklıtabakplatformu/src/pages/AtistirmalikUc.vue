<template>
  <div class="kurabiye-container">
    <div class="card-container">
      <div class="kurabiye-card">
        <h1 class="kurabiye-title">Şekersiz Yulaflı Kurabiye</h1>
        <img src="@/assets/kurabiye2.png" alt="Nohut Cipsi" class="card-image">
      </div>
      <div class="kurabiye-card">
        <h2 class="kurabiye-title">Malzemeler</h2>
        <ul class="ingredient-list">
          <li>• 2 adet olgun muz</li>
          <li>• 1 su bardağı yulaf ezmesi</li>
          <li>• 1 yemek kaşığı fıstık ezmesi</li>
          <li>• 1 yemek kaşığı bal</li>
          <li>• 1/2 su bardağı ceviz içi (ince çekilmiş)</li>
          <li>• 1 tatlı kaşığı tarçın</li>
        </ul>
      </div>
      <div class="kurabiye-card">
        <h2 class="kurabiye-title">Yapılışı</h2>
        <ul class="instruction-list">
          <li>• Muzları çatalla ezin ve fıstık ezmesi ile karıştırın.</li>
          <li>• Yulaf ezmesi, bal, ceviz içi ve tarçını ekleyip iyice karıştırın.</li>
          <li>• Karışımdan küçük parçalar alıp kurabiye şekli verin ve yağlı kağıt serili fırın tepsisine dizin.</li>
          <li>• Önceden ısıtılmış 180 derece fırında 12-15 dakika pişirin.</li>
          <li>• Fırından çıkardıktan sonra soğumaya bırakın ve servis edin.</li>
        </ul>
      </div>
...
export default {
  ...
  computed: {
    filteredReviews() {
      return this.reviews.filter(review => review.tarif === 'Şekersiz Yulaflı Kurabiye');
    },
    averageRating() {
      const kurabiyeReviews = this.filteredReviews;
      if (kurabiyeReviews.length === 0) return 0;
      const totalRating = kurabiyeReviews.reduce((acc, review) => acc + review.rating, 0);
      return totalRating / kurabiyeReviews.length;
    },
  },
  methods: {
    setRating(star) {
    ...
    submitReview() {
      if (this.comment && this.rating) {
        const newReview = {
          ...
          tarif: 'Şekersiz Yulaflı Kurabiye'
        };
