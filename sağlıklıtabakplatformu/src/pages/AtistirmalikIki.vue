<template>
  <div class="sufle-container">
    <div class="card-container">
      <div class="sufle-card">
        <h1 class="sufle-title">Tencerede Şekersiz Sufle</h1>
        <img src="@/assets/sufle2.png" alt="Nohut Cipsi" class="card-image">
      </div>
      <div class="sufle-card">
        <h2 class="sufle-title">Malzemeler</h2>
        <ul class="ingredient-list">
          ...
          <li>• 1 çimdik tuz</li>
          <li>• 80 gram bitter çikolata (erimiş)</li>
        </ul>
      </div>
      <div class="sufle-card">
        <h2 class="sufle-title">Yapılışı</h2>
        <ul class="instruction-list">
          <li>• Muzları çatalla ezin ve yumurtaları ekleyip çırpın.</li>
          <li>• Kakao, kabartma tozu ve tuzu ekleyip karıştırın.</li>
          <li>• Erimiş bitter çikolatayı karışıma ekleyin ve iyice karıştırın.</li>
          <li>• Karışımı sufle kaplarına paylaştırın.</li>
          <li>• Tencereye su koyun ve sufle kaplarını suya oturtun.</li>
          <li>• Tencerenin kapağını kapatıp kısık ateşte 15-20 dakika pişirin.</li>
        </ul>
      </div>
...
export default {
  ...
  computed: {
    filteredReviews() {
      return this.reviews.filter(review => review.tarif === 'Tencerede Şekersiz Sufle');
    },
    averageRating() {
      const sufleReviews = this.filteredReviews;
      if (sufleReviews.length === 0) return 0;
      const totalRating = sufleReviews.reduce((acc, review) => acc + review.rating, 0);
      return totalRating / sufleReviews.length;
    },
  },
  methods: {
    setRating(star) {
    ...
    submitReview() {
      if (this.comment && this.rating) {
        const newReview = {
          ...
          tarif: 'Tencerede Şekersiz Sufle'
        };
